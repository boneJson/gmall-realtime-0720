package com.zxk.gmall.realtime.app.dwd.db;

import com.alibaba.fastjson.JSON;
import com.zxk.gmall.realtime.bean.OrderInfoRefundBean;
import com.zxk.gmall.realtime.util.MyKafkaUtil;
import com.zxk.gmall.realtime.util.MysqlUtil;
...
//        System.setProperty("HADOOP_USER_NAME", "atguigu");
        ...
        DataStream<OrderInfoRefundBean> orderInfoRefundBeanDataStream = tableEnv.toAppendStream(orderInfo, OrderInfoRefundBean.class);

        // TODO 6. 过滤符合条件的订单表退单数据
        SingleOutputStreamOperator<OrderInfoRefundBean> filterStream = orderInfoRefundBeanDataStream.filter(new FilterFunction<OrderInfoRefundBean>() {
            @Override
            public boolean filter(OrderInfoRefundBean orderInfoRefundBean) throws Exception {
                String old = orderInfoRefundBean.getOld();
                if (old != null) {
        ...
        // TODO 7. 将订单表退单流转化为表
        Table orderInfoRefund = tableEnv.fromDataStream(filterStream);
